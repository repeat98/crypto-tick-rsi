...
def parse_args():
    p = argparse.ArgumentParser(
        description="Preprocess, label & train 2-channel CNN with GASF+MTF on 1m K-line CSVs"
    )
    ...
    p.add_argument('--image_size', type=int,   default=None, help="Resize GAF/MTF to this size")
    p.add_argument('--batch_size', type=int,   default=32)
    p.add_argument('--epochs',     type=int,   default=20)
    p.add_argument('--lr',         type=float, default=1e-4)
    p.add_argument('--wd',         type=float, default=1e-5)
    p.add_argument('--workers',    type=int,   default=os.cpu_count())
    p.add_argument('--model_out',  default='best_2c_klines.pth')
    return p.parse_args()
...
def preprocess(input_dir, data_dir, freq, window, stride, image_size):
    """
    Build 2-channel GASF+MTF per sliding window.
    """
    ...
    for fp in tqdm(sorted(Path(input_dir).glob("*.csv")), desc="Preprocess"):
        ...
        for i in range(0, n - w_bars + 1, s_bars):
            ...
            # MTF
            m = mtf.fit_transform(a.reshape(1,-1))[0]

            # optional resize both
            if image_size and g.shape[0] != image_size:
                ...
                im = Image.fromarray(((m+1)/2*255).astype(np.uint8))
                im = im.resize((image_size,image_size), Image.BILINEAR)
                m = (np.array(im, np.float32)/127.5) - 1.0

            tensor = np.stack([g.astype(np.float32), m.astype(np.float32)], axis=0)
...
class KlineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = torch.from_numpy(arr).float()   # shape (2,H,W)
        y = int(row.label) + 1
        return x, y
...
def train(labels_dir, data_dir, args):
    ...
    # adapt to 2-channel input
    orig = model.features[0][0]
    model.features[0][0] = nn.Conv2d(2, orig.out_channels,
