...
def parse_args():
    p = argparse.ArgumentParser(
        description="Preprocess, label & train 5-channel GAF CNN on 1m K-line CSVs"
    )
    ...
    p.add_argument('--model_out',  default='best_5c_klines.pth')
    return p.parse_args()
...
def preprocess(input_dir, data_dir, freq, window, stride, image_size):
    """
    Build 6-channel GASF per sliding window: [close, MA8, MA20, RSI14, volume, ATR14]
    """
    ...
    for fp in tqdm(sorted(Path(input_dir).glob("*.csv")), desc="Preprocess"):
        ...
        avg_gain = gain.rolling(window=14, min_periods=14).mean().fillna(0.0)
        avg_loss = loss.rolling(window=14, min_periods=14).mean().fillna(0.0)
        rs = avg_gain / (avg_loss + 1e-8)
        feat['RSI14'] = (100 - 100/(1+rs)).fillna(0.0)
        ...
        arrays = {
            ...
            'MA8':    feat['MA8'].to_numpy(dtype=np.float32),
            'MA20':   feat['MA20'].to_numpy(dtype=np.float32),
            'RSI14':  feat['RSI14'].to_numpy(dtype=np.float32),
            'volume': feat['volume'].to_numpy(dtype=np.float32),
            'ATR14':  feat['ATR14'].to_numpy(dtype=np.float32),
        }
        n = len(feat)
        ...
        for i in range(0, n - w_bars + 1, s_bars):
            ...
            tensor = np.stack(channels, axis=0)  # shape (6, H, W)
...
class KlineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = torch.from_numpy(arr).float()         # (5, H, W)
...
def train(labels_dir, data_dir, args):
    ...
    # adapt input conv to 6 channels
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for x, y in pbar:
            ...
            running_total += bs
            pbar.set_postfix({'mse': f"{running_loss/running_total:.6f}"})
