...
def train_one_epoch(model, loader, criterion, optimizer, scaler, device):
    ...
    running_correct = 0
    running_total = 0
    # Determine autocast dtype: float16 for CUDA, bfloat16 for MPS
    autocast_dtype = torch.float16 if device.type == 'cuda' else torch.bfloat16 if device.type == 'mps' else None
    for x, y in pbar:
        x, y = x.to(device), y.to(device)
        optimizer.zero_grad()
        with torch.autocast(device_type=device.type, dtype=autocast_dtype, enabled=(scaler is not None or device.type=='mps')):
            out = model(x)
...
def main():
    ...
    # JIT compile for speed (PyTorch 2.0) only on CUDA
    if device.type == 'cuda':
        model = torch.compile(model)
    else:
        print(f"Skipping torch.compile on device {device}")
    # Use channels-last for optimized conv performance
    ...
    if device.type == 'cuda':
        ...
        scaler = None  # BF16 doesn't require scaling
    else:
        scaler = None
