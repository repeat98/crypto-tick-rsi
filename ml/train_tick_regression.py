...
def parse_args():
    ...
    p.add_argument('--t0',         type=int, default=5,
                   help='Number of epochs for first restart in CosineAnnealingWarmRestarts')
    return p.parse_args()
...
def train(labels_dir, data_dir, args):
    ...
    scheduler_cos = CosineAnnealingWarmRestarts(optimizer, T_0=args.t0, T_mult=1, eta_min=args.lr*1e-2)
    # fallback reduce lr on plateau
    scheduler_plateau = ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=3, min_lr=1e-7)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for x, y in pbar:
            ...
            scheduler_cos.step(epoch + pbar.n / len(train_loader))
        ...
        # plateau scheduler on validation
